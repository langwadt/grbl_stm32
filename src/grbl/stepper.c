...
//      OCR1A = st.exec_segment->cycles_per_tick;
      set_timerperiod(st.exec_segment->cycles_per_tick);
...
{
	GPIO_WriteBit(STPX, outbits&(1<<X_STEP_BIT));
	GPIO_WriteBit(STPY, outbits&(1<<Y_STEP_BIT));
	GPIO_WriteBit(STPZ, outbits&(1<<Z_STEP_BIT));
}


uint32_t timerrunning=0;
